...
- [X] T002 [P] Extend `tests/database/helpers/fixtures.ts` with the Phase 6 fixture constants as new exports (existing exports keep their shapes): the two seeded open sessions (Downtown T1 and T2) with their ids, restaurant/branch/table bindings, opened-at invariants, and participants; the **deterministic dev tokens** (documented plaintext constants in the file, their SHA-256 hashes computed by the seed) and the expectation that each resolves to its session; Downtown's table-label map — the single fixture source shared by the seed task (T005), the schema suite, and every story's suites — FR-022, SC-007, data-model.md
