...
- `sessionClient.ts` — the typed wrapper over the seven RPCs and the **only** import path for them in application code (the 005/006 pattern): one RPC round trip per call, no authorization logic, no optimistic writes; maps `42501` → `AuthorizationError` and `P0001` → `SessionError` with the server message verbatim; parses the jsonb payloads into typed structures (`TaxPayloadError`-style: `SessionPayloadError` on malformed shapes).
