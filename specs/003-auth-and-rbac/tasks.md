...
- [X] T025 [US3] Rewire `src/app/router.tsx` to the route surface of contracts/auth-client.md: public `/`, `/r/:restaurantSlug`, `/order/:branchId`, `/signin`; `RequireStaff`-guarded `/dashboard`, `/dashboard/profile`, `/dashboard/staff`; `RequireSuperAdmin`-guarded `/admin` — unauthenticated entry redirects with return-to (FR-013) and deep links to unauthorized views hit the guards' `NotAuthorized` (FR-014) (depends on T019–T024) — **Delivered note (recorded post-hoc 2026-09-15)**: the Phase 0 e2e baseline `e2e/routes.test.ts` was also updated in this task (26 lines, commit 56864af): unauthenticated `/dashboard` and `/admin` now assert the redirect to `/signin` (FR-013), superseding plan.md's "unchanged" claim for that file
- [X] T026 [P] [US3] Create `tests/unit/auth.guards.test.ts` — the guard/route-permission decision matrix (plan.md Testing; research.md §10): unauthenticated ⇒ redirect to `/signin` carrying the requested location; signed-in unlinked identity ⇒ `NotAuthorized` on every staff route; per-role reachability — owner (all staff views incl. the staff list), branch manager (own-branch scope + the staff list), cashier/kitchen (own-branch scope, staff list denied), multi-membership member (union of scopes), super admin (`/admin` only); `canReadStaffList` predicate truth table. `npm run test:unit` exits 0 — FR-013/FR-014/FR-015 (depends on T018–T019; parallel with T020–T025) — **Delivered deviation (recorded post-hoc 2026-09-15)**: the suite was delivered as `tests/unit/auth.guards.test.tsx` (not `.ts` — JSX test components under a static server-side-rendering strategy, documented in the file header), and `vitest.config.ts` gained `tests/**/*.test.tsx` in its include (commit 56864af); plan.md's Project Structure listing corrected to `.tsx` accordingly
...
- [X] T039 Final commit and push of all Phase 2 artifacts (three migrations, seed, scripts, the `src/features/auth/` module + routes, test suites, regenerated types, docs, spec artifacts) to GitHub `main` — mirrors feature 002 T018 (depends on T036–T038)
...
- [X] T040 Restore the feature's quality gate: reformat `docs/development.md` so `npm run format:check` — and therefore `npm run verify` — exits 0. The T034 edit of the Prerequisites table (`anon key` → `publishable key`) left the file unformatted; the committed state fails at the pipeline's first step, contradicting the gate recorded green in T036 and the `docs/development.md` Setup step 9 statement that `verify` must pass — per T036 (partial). **Resolved 2026-09-16**: `docs/development.md` reformatted (whitespace-only — the Prerequisites table's column widths, grown by the T034 `anon key` → `publishable key` edit); `npm run format:check` exits 0 and the full `npm run verify` chain exits 0
- [X] T041 Harden the cold-start determinism of the FR-013 return-to checks in `e2e/auth.routes.test.ts` (e.g. an explicit assertion timeout in `playwright.config.ts` or the test, or a post-sign-in navigation in `src/routes/SignInPage.tsx` that does not wait for the context RPC to settle) so `npm run test:e2e` exits 0 on the first run of a session — 2/16 failed on a cold first run (signed-in page still on `/signin` at the 5 s default expect deadline; the two first-executed tests), 2/2 and 16/16 green on warm re-runs — per T027/T030 (SC-006) (partial). **Resolved 2026-09-16**: root cause confirmed — the cold-run sign-in chain (the Auth API round trip + the `current_auth_context` RPC + the dev server's first-load module transform) exceeds the 5 s default expect deadline (the two FR-013 tests, always the first executed, measured 4.8–7.8 s end-to-end per cold invocation); least-invasive fix applied in the test/config layer — an explicit `expect.timeout: 15_000` in `playwright.config.ts`; `src/routes/SignInPage.tsx`'s post-sign-in navigation is unchanged (waiting for the context is what picks `/admin` vs `/dashboard`) and no assertion was weakened; cold `npm run test:e2e` green 16/16 on two consecutive cold invocations
